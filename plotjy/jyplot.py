...
from org.meteothink.form import ChartForm
from org.meteothink.legend import LegendManage, LegendScheme, LegendType
from org.meteothink.image import AnimatedGifEncoder
        ...
        if len(locs) > 0:
            ...
            if isinstance(locs[0], datetime.datetime):
                for i in range(len(locs)):
                    locs[i] = np.miutil.date2num(locs[i])
        gca.set_xticks(locs)
        args = args[1:]
    if len(args) > 0:
        labels = args[0]
