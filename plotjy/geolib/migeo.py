...
from org.meteothink.data.mapdata.geotiff import GeoTiff
from org.meteothink.shape import ShapeUtil, PolygonShape
from org.meteothink.legend import BreakTypes
...
def maskout(data, mask, x=None, y=None):
    ...
    if mask is None:
        ...
        r = GeoComputation.maskout(data.asarray(), mask.asarray())
    ...
    if not isinstance(mask, (list, ArrayList)):
        mask = [mask]
    r = GeoComputation.maskout(data._array, x._array, y._array, mask)
    if isinstance(data, DimArray):
        return DimArray(r, data.dims, data.fill_value, data.proj)
    else:
        return np.NDArray(r)
