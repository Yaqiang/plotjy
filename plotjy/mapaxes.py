...
from org.meteothink.chart import ChartScaleBar, ChartNorthArrow
from org.meteothink.chart.plot import MapPlot, GraphicFactory
from org.meteothink.math import ArrayUtil
from org.meteothink.data.meteodata import DrawMeteoData
from org.meteothink.map import MapView
from org.meteothink.legend import BreakTypes, LegendManage, LegendScheme, LegendType
from org.meteothink.shape import Shape, PolylineShape, PolygonShape, ShapeTypes, Graphic
from org.meteothink.projection.info import ProjectionInfo
from org.meteothink.common import Extent
from org.meteothink.layer import LayerTypes, WebMapLayer
from org.meteothink.data.mapdata.webmap import WebMapProvider
from org.meteothink.layout import ScaleBarType
...
class MapAxes(Axes):
    ...
    def loadmip(self, mipfn, mfidx=0):
        '''
        Load one map frame from a meteothink project file.
        
        :param mipfn: (*string*) meteothink project file name.
    ...
    def plot_bak(self, *args, **kwargs):
        ...
        if n == 1:
            ...
            if isinstance(args[0], DimArray):
                ...
                for i in range(0, len(args[0])):
                    xdata.append(i)
            xdatalist.append(np.asarray(xdata).array)
            ydatalist.append(np.asarray(ydata).array)
        elif n == 2:
            if isinstance(args[1], basestring):
                ydata = plotutil.getplotdata(args[0])
                if isinstance(args[0], DimArray):
            ...
            for arg in args: 
                if c == 'x':    
                    xdatalist.append(np.asarray(arg).array)
                    c = 'y'
                elif c == 'y':
                    ydatalist.append(np.asarray(arg).array)
                    c = 's'
                elif c == 's':
                    if isinstance(arg, basestring):
                        styles.append(arg)
    ...
    def scatter(self, *args, **kwargs):
        ...
        if ls is None:
            ...
            if not size is None:
                if isinstance(size, (list, tuple, np.NDArray)) and len(size) == x.size:
                    isunique = True
            marker = kwargs.get('marker', None)
            if not marker is None:
                if isinstance(marker, (list, tuple, np.NDArray)) and len(marker) == x.size:
                    isunique = True
            if isunique:
    ...
    def imshow(self, *args, **kwargs):
        ...
        if isrgb:
            ...
            if len(args) > 0:
                ...
                if ls is None:
                    ls = LegendManage.createImageLegend(a, cmap)
            plotutil.setlegendscheme(ls, **kwargs)
            ...
            layer = DrawMeteoData.createRasterLayer(x._array, y._array, a._array, 'layer', ls) 
    ...
    def quiver(self, *args, **kwargs):
        ...
        cdata = None
        onlyuv = True
        if n >= 4 and isinstance(args[3], (DimArray, np.NDArray)):
            onlyuv = False
        if onlyuv:
            u = np.asarray(args[0])
            v = np.asarray(args[1])
